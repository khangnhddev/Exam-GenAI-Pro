  ...
  <div>
    ...
    <div v-if="user" class="mt-8">
      ...
      <div v-if="activeTab === 'certificates'" class="mt-6">
        <div v-if="userActivity.certificates.length > 0" class="bg-white shadow overflow-hidden sm:rounded-lg">
          <!-- Add your certificates content here -->
          <ul class="divide-y divide-gray-200">
            <li v-for="cert in userActivity.certificates" :key="cert.id" class="px-4 py-4 sm:px-6">
              <!-- Certificate content -->
            </li>
          </ul>
        </div>
        <div v-else class="text-center py-12 bg-white shadow sm:rounded-lg">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900">No certificates earned</h3>
          <p class="mt-1 text-sm text-gray-500">This user hasn't earned any certificates yet.</p>
        </div>
      </div>
    </div>
  </div>
...
const loadUser = async () => {
  try {
    ...
    userActivity.value = response.data.activity
  } catch (error) {
